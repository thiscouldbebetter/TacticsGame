...
class WorldExtended extends World {
        ...
        var actionMovePerform = (universe, worldAsWorld, direction) => {
            ...
            if (targetPos == null) {
                ...
                if (moverOrientation.equals(direction) == true) {
                    var moverPosNext = moverActive.pos.clone().add(direction).trimToRangeMax(world.map.sizeInCellsMinusOnes);
                ...
                var targetPosNext = targetPos.clone().add(direction).trimToRangeMax(world.map.sizeInCellsMinusOnes);
            ...
            (universe, world) => // perform
             {
                ...
                if (moverActive.targetPos == null) {
                    ...
                    var moverTarget = world.moverAtPos(moverActive.targetPos);
                    if (moverTarget != null) {
    ...
    initialize(universe) {
        ...
        [
            ...
            // children
            [
                ControlLabel.fromPosAndText(Coords.fromXY(5, 5), // pos
                DataBinding.fromGet((c) => world.moverActive().factionName)),
                ControlLabel.fromPosAndText(Coords.fromXY(5, 15), // pos
                DataBinding.fromGet((c) => world.moverActive().defnName)),
                ControlLabel.fromPosAndText(Coords.fromXY(5, 25), // pos
                DataBinding.fromGet((c) => {
                    var moverActive = world.moverActive();
                    var moverDefn = moverActive.defn(world);
                    return "Health:" + moverActive.integrity + "/" + moverDefn.integrityMax;
                })),
                ControlLabel.fromPosAndText(Coords.fromXY(5, 35), // pos
                DataBinding.fromGet((c) => {
                    var moverActive = world.moverActive();
                    var moverDefn = moverActive.defn(world);
    ...
    draw(universe) {
        ...
        var world = this;
        display.clear();
        display.drawRectangle(Coords.Instances().Zeroes, display.sizeInPixels, Color.byName("White"), Color.byName("Gray"), null // areColorsReversed
        );
        var map = world.map;
        map.draw(display);
        ...
        world.containerMain.draw(universe, display, drawLoc, null // style
        );
    }
    toControl() {
