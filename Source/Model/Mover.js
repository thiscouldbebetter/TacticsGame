...
class Mover {
    ...
    draw(universe, world, display, map, isMoverActive) {
        ...
        drawPos.overwriteWith(mover.pos).multiply(mapCellSizeInPixels).add(map.pos).add(mapCellSizeInPixelsHalf);
        ...
        display.drawCircle(drawPos, radius, mover.faction(world).color, colorStroke, 1 // borderThickness
        );
        drawPos2.overwriteWith(mover.orientation).multiplyScalar(radius).add(drawPos);
        display.drawLine(drawPos, drawPos2, colorStroke, 1);
        drawPos.subtract(mapCellSizeInPixelsHalf);
        display.drawText(" " + moverDefn.codeChar, null, // fontHeight
        drawPos, colorStroke, null, null, null, null // ?
        );
        if (isMoverActive) {
            if (this.targetPos != null) {
                drawPos.overwriteWith(this.targetPos).multiply(mapCellSizeInPixels).add(map.pos).add(mapCellSizeInPixelsHalf);
                display.drawCircle(drawPos, radius / 2, colorStroke, Color.byName("Red"), 1 // borderThickness
                );
            }
        }
    }
